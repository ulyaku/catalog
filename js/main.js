...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ── CONTACT FORM ── */
  updateContactPanel();   // render initial state on page load

  var form = document.getElementById('contact-form');
  if (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      // Build submission summary for console / future backend
      var submission = {
        ...
        quiz:   quizState.completed ? {
          ...
          answers: quizState.answers.map(function (a, i) {
            return (i + 1) + '. ' + (a.skipped ? '[ATLANDI]' : (a.correct ? '[DOGRU]' : '[YANLIS: ' + a.chosen + ']')) + ' — ' + a.q;
          })
        } : 'Quiz tamamlanmadı'
      };
      console.log('Form gönderildi:', submission);
