...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ════════════════════════════════════════
     ...
     Reads SITE object and fills all page IDs
  ════════════════════════════════════════ */
  ...
  // General
  document.title = SITE.general.title;
  set('nav-logo', SITE.general.name);

  // Footers (same across all pages)
  var footerText = SITE.general.name + ' — ' + SITE.general.footer_text;
  ['home','method','contact'].forEach(function(p) {
    set('footer-logo-' + p, footerText);
    set('footer-copy-' + p, SITE.general.footer_copy);
  });

  // Hero
  set('hero-title', SITE.hero.title.replace('\n', '\n'), false);
  document.getElementById('hero-title').innerHTML = SITE.hero.title.replace('\n', '<br>');
  set('hero-para1', SITE.hero.para1);
  set('hero-para2', SITE.hero.para2);

  // About
  set('about-statement', SITE.about.statement);
  set('about-bio', SITE.about.bio, true);

  // Method cards
  var methodGrid = document.getElementById('method-grid');
  if (methodGrid) {
    methodGrid.innerHTML = SITE.methods.map(function(m, i) {
  ...
  if (whoGrid) {
    whoGrid.innerHTML = SITE.audiences.map(function(a) {
  ...
  if (stepsEl) {
    stepsEl.innerHTML = SITE.steps.map(function(s, i) {
  ...
  // Belief text
  set('belief-text', SITE.belief, true);

  // Pricing
  var pricingEl = document.getElementById('pricing-grid');
  if (pricingEl) {
    pricingEl.innerHTML = SITE.pricing.map(function(p) {
  ...
  // Contact
  set('contact-title', SITE.contact.title);
  set('contact-para1', SITE.contact.para1);
  set('contact-para2', SITE.contact.para2);
  set('contact-location', '📍 ' + SITE.general.location);
  set('contact-email', '📧 ' + SITE.general.email);
  var igEl = document.getElementById('contact-instagram');
  if (igEl) {
    igEl.href = SITE.general.instagram;
    igEl.textContent = '@' + SITE.general.instagram.split('/').pop();
  }
  set('free-label', SITE.contact.free_label);
  set('free-text', SITE.contact.free_text);
  set('form-thanks', SITE.contact.thanks_msg);
  ...
  ════════════════════════════════════════ */
  var etQuestions = {
    ilkokul:  SITE.quiz.level1,
    ortaokul: SITE.quiz.level2,
    lise:     SITE.quiz.level3
  };
  ...
  if (form) {
    ...
    function clearErrors() {
      ...
      form.querySelectorAll('.form-input').forEach(function(e) { e.style.borderColor = ''; });
    }
    ...
    function validate() {
      ...
      var age   = document.getElementById('field-age');
      if (!name || !name.value.trim()) { showFieldError('field-name', 'Öğrencinin adını girin.'); ok = false; }
      if (!age || !age.value) { showFieldError('field-age', 'Yaş grubunu seçin.'); ok = false; }
      if (!email || !email.value.trim()) {
        showFieldError('field-email', 'E-posta adresinizi girin.'); ok = false;
      } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value.trim())) {
        showFieldError('field-email', 'Geçerli bir e-posta adresi girin.'); ok = false;
      }
      return ok;
    }
    ...
    function buildEmailBody() {
      ...
      var quizSection = quizState.completed
        ? '\n\n--- QUIZ SONUCU ---\nSeviye: ' + (quizState.level === 'ilkokul' ? 'Level 1' : quizState.level === 'ortaokul' ? 'Level 2' : 'Level 3') +
    ...
    form.addEventListener('submit', function(e) {
      ...
      var url = (typeof SITE !== 'undefined') ? SITE.general.formspree_url : '';

      if (!url || url.indexOf('YOUR_FORM_ID') !== -1) {
        submitBtn.disabled = false;
        submitBtn.textContent = 'Mesaj Gönder →';
        thanks.style.color = 'var(--terra)';
        thanks.textContent = '⚠ Formspree henüz kurulmadı. content.js dosyasına formspree_url ekleyin.';
      ...
      .catch(function() {
        ...
        submitBtn.textContent = 'Mesaj Gönder →';
        thanks.style.color = 'var(--terra)';
        thanks.textContent = '✗ Bir hata oluştu. Lütfen tekrar deneyin ya da doğrudan e-posta gönderin: ' + SITE.general.email;
        thanks.classList.add('show');
      });
    });
  }
